...
- OLTP: PostgreSQL (normalized schemas per module; shared database with schema-level boundaries)
- Caching: Redis for hot paths (availability, session, rate limits)
- Files/Artifacts: object storage for exports/imports and ML artifacts
...
- External integration adapters can subscribe/emit as needed.
...
- Move from shared DB to database-per-service when necessary; keep contracts stable.
- Introduce a durable external broker (e.g., Kafka/Redpanda) once event volume or consumers warrant it.
