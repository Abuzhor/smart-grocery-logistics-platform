...
- Shared PostgreSQL with schema boundaries; Redis for caching
- Domain events and an outbox pattern to decouple modules and enable integrations
...
- Negative: shared database can become a bottleneck; strong discipline needed to keep boundaries clean
...
- Team ownership boundaries that require independent deploy cadence
...
- Treat the database as an implementation detail of each module

## Decision Owner
Core architecture group and service/module owners
